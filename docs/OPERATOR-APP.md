...
| Right pane | Tabs: **USB console** and **Apple //c**. Launching //c selects the Apple tab. Operator records the pane geometry and starts MAME with `SDL_VIDEO_WINDOW_POS` / `MEGAFLASH_MAME_SIZE` so the window opens over the tab even before Accessibility can move it. Docking then uses AppleScript (`unix id` of `mame`). Grant **Accessibility** to **MegaFlash Operator.app** (or the `tauri dev` binary), not only Terminal. SDL windows with no AX UI stay separate; the hint in the Apple tab shows the osascript error. **Stop //c** signals the whole process group (bash wrapper + MAME + overlay Bramble). |
