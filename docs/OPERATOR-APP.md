...
| Right pane | Tabs: **USB console** and **Apple //c**. Launching //c selects the Apple tab. Operator records the pane geometry (`SDL_VIDEO_WINDOW_POS`) and then **moves MAME with in-process Accessibility APIs** (not `osascript` — System Events does not inherit this app’s TCC grant). MAME is still an SDL OS window (title bar remains) overlaid on the pane. Failures go to `~/Library/Logs/MegaFlashOperator/mame-win.log`. **Stop //c** signals the whole process group. |
