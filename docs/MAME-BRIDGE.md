...
10b. **CP Format Error `$ba` / IIc disk I/O stall** — Format “Erase the drive” called `tsEraseFlashDisk` (512× 64KB SPI erase + `WaitUntilBusyClear`). `InitSpi` is skipped, so erase hung; BUSY unstick left PARAM garbage (`$ba`) and could leave the flash mutex held so later SmartPort reads (Total Replay scrolling) stall. Host-complete erase to `0xFF` on the SPI backing files. TFTP overwrite already used `WriteBlockForImageTransfer` and did not hit this path.
