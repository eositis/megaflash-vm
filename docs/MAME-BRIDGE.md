...
10b. **CP Format Error `$ba` / IIc disk I/O stall** — Format “Erase the drive” is `CMD_ERASEDISK` → RAM veneer `__DoEraseDisk` (`0x20004410`). The veneer rewrite used to consume the cpu_step so the `tsEraseFlashDisk` entry stub never ran; native SPI hang + BUSY unstick left PARAM as `$ba` (`:` with high bit) and core1 PC `0xFFFFFFFE`. Host-complete `DoEraseDisk` (fill unit with `0xFF`) at the veneer and flash entry. TFTP overwrite already used `WriteBlockForImageTransfer`.
