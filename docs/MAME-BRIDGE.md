...
**Uthernet II / Telnet65 / wget / A2Stream:** Lua taps `$C0C4–$C0C7`. Overlay host-completes the W5100 window. Local DHCP is Ethernet+IP broadcast with `yiaddr=192.168.4.3`. DNS is userspace UDP NAT. **IP65 TCP (telnet/wget) is MACRAW + TAP TCP NAT.** **A2Stream** (`-D SINGLE_SOCKET`) uses W5100 **hardware TCP on socket 0** with 8KB RX and AI wrap `$7FFF→$6000`. Host `recv()` must not run on every DATA read — that tears `Sn_RX_RSR` and the PWM player sounds like packets arrived out of order. TAP NAT must not `recv()` faster than Contiki can RECV (wget also writes MegaFlash). Sniff returns 0 when RX is full; TAP holds the segment and logs `[TAP] TCP NAT pause (W5100 RX full)`.
...
**TFTP stalls at Transferring / Blocks:1 after ACK:** Under emu, TAP flooded the guest with DATA1 retransmits + server `Timeout` before `PumpNetworkIteration` could ACK; late ACK got `Unknown transfer ID` and no block 2. Even pacing one DATA was not enough — guest ACK still arrived after tftpd closed. TAP now **host-ACKs** the server immediately, buffers blocks, paces delivery to the guest, and suppresses forwarding guest ACKs (already ACKed on the wire).
...
5. **Hashtable rehash OOM (~1GB `operator new`)** — after WiFi/DHCP in RunTFTP, `AddSession` → `_M_need_rehash` / `_M_next_bkt` SoftFloat + UDIV under Thumb emu yields absurd bucket counts → `new(~1074038716)` → OOM → Status stays Idle. During RunTFTP bump-malloc: host-complete need_rehash/next_bkt/allocate_buckets; clamp `_M_rehash` n; reject oversized bump allocs; recover bucket `new` from lr `0x1000147a` to 11 buckets.
...
10b. **CP Format Error `$ba` / `$a0` / IIc disk I/O stall** — Erase is `DoEraseDisk` (RAM veneer). Format then runs `DoFormatDisk` (`$a0` = leftover PARAM, high-bit space) via the same veneer miss; Total Replay catalog I/O is `DoReadBlock` in SRAM and can hang on native SPI. Host-complete erase, format (22 ProDOS blocks), and SmartPort read/write commands.
