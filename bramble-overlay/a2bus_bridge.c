...
static int g_long_cmd_depth;
static const char *g_long_cmd_why;
...
void a2bus_long_cmd_begin(const char *why)
{
    ...
    if (why)
        g_long_cmd_why = why;
    if (g_long_cmd_depth == 1) {
        fprintf(stderr, "[A2Bus] long CMD begin (%s) — BUSY unstick suppressed\n",
                why ? why : "?");
        fflush(stderr);
    }
}
...
{
    if (g_long_cmd_depth > 0)
        g_long_cmd_depth--;
    if (g_long_cmd_depth == 0) {
        fprintf(stderr, "[A2Bus] long CMD end (%s)\n",
                g_long_cmd_why ? g_long_cmd_why : "?");
        fflush(stderr);
        g_long_cmd_why = NULL;
    }
}
...
static void a2bus_unstick_busloop(const char *why)
{
    ...
    if ((st & MF_BUSYFLAG) != 0 && a2bus_busy_is_long_command()) {
        static unsigned skip_logged;
        if (skip_logged < 8u || (skip_logged % 200u) == 0u) {
            fprintf(stderr,
                    "[A2Bus] skip unstick (%s) — long cmd core1PC=0x%08X depth=%d\n",
                    why, c1pc, a2bus_long_cmd_active());
            fflush(stderr);
        }
        skip_logged++;
